<template>
    <div class="tagProject-view">
        <div class="header-nav">
            <div :class="[isProjectsChats? 'active': '', 'nav-handler']" @click="getOrganization('chat')" >群聊</div>
            <div :class="[isProjectsGroups? 'active': '', 'nav-handler']" @click="getOrganization('group')" >评论</div>
        </div>
        
        <search v-show="isProjectsChats" :match="match"></search>
        <div v-show="!isOnLoading"  class="itemslists">
            ...
            <ul class="items-list cont-scrollbar" v-if="isProjectsChats">
                <!-- <div v-if="projectsChatsFilter.length">
                    <div class="items-title">我的任务组</div>
                ...
                <div v-for="(organization, ind) in organizationsIds" :key="ind" >
                    <div class="items-title" :class="{itemtop:organization.name}" >{{organization.name?organization.name:'我的任务组'}}</div>
                    
                    <div v-if="projectsChatsFilter[ind]">
                    <select-cell v-for="(item, index) in projectsChatsFilter[ind]"
                        :key="index" 
                        :typeSwitch="'projectChat'"
                        :item="item"
                ...
                <Col class="thrCol" span="6">
                    ...
                    <ul class="items-list cont-scrollbar borderright">
                        ...
                        <div v-for="(organization, ind) in organizationsIds" :key="ind" >
                            ...
                            <div v-if="projectsGroupsFilter[ind]">
                            <item-group v-for="(item, index) in projectsGroupsFilter[ind]"
                                :key="index" 
                                :typeSwitch="'projectGroup'"
                                :item="item"
                                @select="getTasksGroups(item._id)"></item-group>
...
export default {
    data () {
        return {
            ...
            promiseList: [], //合并请求
            organizationsIds: [], // 个人和组织的id
            organizationsLists: [], // 个人和组织的任务组列表
            organizationId:'',
            projectsChatsList: [],
            projectsGroupsList: [],
            tasksGroupsList: [],
    ...
    computed:{
        projectsChatsFilter() {
            ...
            this.projectsChatsList.forEach((list, index) =>{
                _data[index] = list.filter(ele =>{
                    return ele.name.match(res)
                })
            })
            // return this.projectsChatsList
            return _data
        },
        projectsGroupsFilter() {
            ...
            this.projectsGroupsList.forEach((list, index) =>{
                _data[index] = list.filter(ele =>{
                    return ele.name.match(res)
                })
            })
            // return this.projectsGroupsList
            return _data
        },
        tasksGroupsFilter() {
            let res = new RegExp(this.searchKey)
    ...
    methods: {
        ...
        getOrganization(type = 'chat') {
            ...
            .then(organization => {
                ...
                this.organizationsIds.push({_id:''}) //个人任务组没有组织id
                ...
                if (type==='chat') {
                    ...
                    // 合并请求
                    this.organizationsIds.forEach( (item, index) => {
                        this.promiseList[index] = this.getProjectsChats(item._id, index)
                    });
                    Promise.all(this.promiseList).then((result) => {

                        result.forEach( (item, index) => {
                            // this.projectsChatsList[index] = item
                    ...
                    // 合并请求
                    this.organizationsIds.forEach( (item, index) => {
                        this.promiseList[index] = this.getProjectsGroups(item._id, index)
                    });
                    Promise.all(this.promiseList).then((result) => {

                        result.forEach( (item, index) => {
                            // this.projectsGroupsList[index] = item
        ...
        getProjectsChats(id, index) {
            ...
            //     this.organizationId = ''
            // }
            // projectsChatsAPI.getProjectsChats(id)
            // .then(projectsChats => {
            //     userAPI.getUser()
            //     .then(user => {
            //         store.state.user = user
            //         this.projectsChatsList[index] = []
            //         projectsChats.forEach( item => {
            //             if (item._id === user._id) return
            //             this.projectsChatsList[index].push(item)
            //         });
            //         this.isOnLoading = false
            //     })
            // })
            return new Promise( (resolve, reject) => {
                projectsChatsAPI.getProjectsChats(id)
                .then(projectsChats => {
                    resolve(projectsChats)
                })
            });
        },

        // 任务组
        getProjectsGroups(id, index) {
            // this.init()
            ...
            //     this.organizationId = ''
            // }
            // projectsGroupsAPI.getProjectsGroups(id)
            // .then(projectsGroups => {
            ...
            //     .then(user => {
            //         store.state.user = user
            //         this.projectsGroupsList[index] = []
            //         projectsGroups.forEach( item => {
            //             if (item._id === user._id) return
            //             this.projectsGroupsList[index].push(item)
            //         });
            //         this.isOnLoading = false
            //     })
            // })
        ...
        selectItemEvent() {
            // console.log('store.state.selectedItem11: ')
